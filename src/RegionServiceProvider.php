...
class RegionServiceProvider extends ServiceProvider
{
    ...
    protected function bindMacroService(): void
    {
        ...
        if (!SimpleService::hasMacro('queryDistance')) {
            SimpleService::macro('queryDistance', function (SimpleService $service, float $lat, float $lng, float $maxDistance = 50, string $geoColumn = 'location') {
                $distanceService = new DistanceService;
                return $distanceService->queryDistance($service, $lat, $lng, $maxDistance, $geoColumn);
            });
        }

        if (!SimpleService::hasMacro('selectDistance')) {
            SimpleService::macro('selectDistance', function (SimpleService $service, float $lat, float $lng, string $geoColumn = 'location', string $alias = 'distance') {
                $distanceService = new DistanceService;
    ...
    {
        $regionPath = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . 'cities.json';
        $this->app->bind('region', fn() => new \SimpleCMS\Region\Packages\Region($regionPath));
    }
